    ...
    process: (bot, msg, suffix) => {
        ...
        axios.get(URL).then(response => {
            if (response.status == 200 && response.data.cod !== '404') { //This should be cod, the API is stupid and doesnt return code
                ...
                bot.createMessage(msg.channel.id, `🌎 __**Weather for ${weath.name === '' ? '' : weath.name+', '}${weath.sys.country}:**__ • (*${weath.coord.lat}, ${weath.coord.lon}*)
...
**🌄 Sunrise:** ${moment(weath.sys.sunrise * 1000).format('HH:mm')} UTC **|** **🌇 Sunset:** ${moment(weath.sys.sunset * 1000).format('HH:mm')} UTC`);
            } else bot.createMessage(msg.channel.id, "There was an error getting the weather for that location. Please try again later.").then(message => utils.messageDelete(bot, message));
