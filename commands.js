...
function loadMainPackageJSON(attempts) {
	attempts = attempts || 1;
	if (attempts > 5) {throw new Error('Can\'t resolve main package.json file');}
	var mainPath = attempts === 1 ? './' : Array(attempts).join("../");
	try {return require.main.require(mainPath + 'package.json');}
	catch (e) {return loadMainPackageJSON(attempts + 1);}
}
...
function correctUsage(cmd) {
	var msg = "Usage: " + prefix[0] + "" + cmd + " " + commands[cmd].usage;
	return msg;
}

var commands = {
	"help": {
		usage: "[command]", description: "Sends a PM containing all of the commands.", delete: true,
		process: function (bot, msg, suffix) {
			if (commands[suffix]) {bot.sendMessage(msg.author, correctUsage(suffix));}
			else {
				var msgArray = [];
				msgArray.push("**Commands: **");
				msgArray.push("```");
				Object.keys(commands).sort().forEach(function (cmd) {
						if (commands[cmd].privateServer && msg.channel.server.id === "87601506039132160") {msgArray.push("" + prefix[0] + "" + cmd + ": " + commands[cmd].description + "");}
						if (!commands[cmd].privateServer) {msgArray.push("" + prefix[0] + "" + cmd + ": " + commands[cmd].description + "");}
					});
				msgArray.push("```");
	...
	"info": {
		usage: "[mentioned user]", description: "Gives info on the server or the user mentioned", delete: true,
		process: function (bot, msg, suffix) {
			if (!suffix) {
				var msgArray = [];
			...
			if (msg.mentions.length === 1 && suffix) {
					...
					msgArray.push("Avatar: " + msg.mentions[0].avatarURL)
					bot.sendMessage(msg, msgArray)
					return;
			}
			if ((suffix && !msg.mentions) || msg.everyoneMentioned) {bot.sendMessage(msg, suffix + " is not a valid user.");}
		}
	},
	"changelog": {
		usage: "[none]", description: "Sends a PM with the Last 5 Changelog Entries", delete: true,
		process: function (bot, msg, suffix) {
	...
	"ping": {
		usage: "[none]", description: "PONG!", delete: false,
		process: function (bot, msg) {bot.sendMessage(msg, "PONG!");}
	},
	"animequote": {
		usage: "[quote number]", description: "Command changed to -quote", delete: true,
		process: function (bot, msg, suffix) {bot.sendMessage(msg.author, "This command was renamed to `-quote`");}
	},
	"quote": {
		usage: "[quote number]", description: "Gives a random anime quote", delete: true,
		process: function (bot, msg, suffix) {
	...
	"pun": {
		usage: "[pun number]", description: "Gives a random pun", delete: true,
		process: function (bot, msg, suffix) {
	...
	"vquote": {
		usage: "[quote message]", description: "Logs message to quotes chat with voice tag (Doesn't work on most servers)", delete: true, privateServer: true,
		process: function (bot, msg, suffix) {
	...
	"tquote": {
		usage: "[quote message]", description: "Logs message to quotes chat with text tag (Doesn't work on most servers)", delete: true, privateServer: true,
		process: function (bot, msg, suffix) {
			if (msg.channel.server.id === "87601506039132160") {
				if (!suffix) {bot.reply(msg, "you'll need to have a quote to quote something, Senpai.");}
	...
	"gif": {
		usage: "[image tags]", description: "returns a random gif matching the tags passed", delete: true,
		process: function (bot, msg, suffix) {
			var tags = suffix.split(" ");
			get_gif(tags, function (id) {
				if (typeof id !== "undefined") {bot.sendMessage(msg, "Using the tags **(Tags: " + (tags ? tags : "Random GIF") + " )** I found the following gif for you Senpai, \nhttp://media.giphy.com/media/" + id + "/giphy.gif ");}
				else {bot.sendMessage(msg, "Invalid tags Senpai, please try something different.");}
			});
		}
	},
	"reddit": {
		usage: "[subreddit] or [none]", description: "Links the top post on /r/all or the top post on the subreddit entered", delete: false,
		process: function (bot, msg, suffix) {
			var path = "/.rss"
	...
	"roll": {
		usage: "[max value]", description: "Rolls a dice with the number entered, if none entered 6 is used", delete: true,
		process: function (bot, msg, suffix) {
			var max = 6;
	...
	"lenny": {
		usage: "[no usage]", description: "Puts a lenny face",delete: true,
		process: function (bot, msg) {bot.sendMessage(msg, "( ͡° ͜ʖ ͡°)");}
	},
	"wewlad": {
		usage: "[no usage]", description: "Sends a wew lad image to the current channel(I'm so sorry)", delete: true,
		process: function (bot, msg) {bot.sendMessage(msg, "https://blazeti.me/wewlad/wewladtoast.png");}
	},
	"faces": {
		usage: "[no usage]", description: "Command Changed to -face ?", delete: true,
		process: function (bot, msg) {bot.sendMessage(msg, "This command was changed to `-face ?`")}
	},
	"face": {
		usage: "[Number between 1-129] or [?] or [none]", description: "Sends a random, specified or a list of Ascii faces", delete: true,
		process: function (bot, msg, suffix) {
			if (suffix && /^\d+$/.test(suffix) && cool.faces.length >= parseInt(suffix - 1)) {bot.sendMessage(msg, cool.faces[suffix - 1]);}
			else if (suffix === "?") {
				var msgArray = [];
	...
	"wiki": {
		usage: "[information to be wiki'd]", description: "Gives you a wikipedia url based on the entered text", delete: true,
		process: function (bot, msg, suffix) {
			if (suffix) {
				new Wiki().search(suffix, 1).then(function (data) {
						new Wiki().page(data.results[0]).then(function (page) {bot.sendMessage(msg, "I searched for **" + suffix + "** and found this Senpai, \n" + page.fullurl)});
					});
			}
			else {bot.sendMessage(msg, "You need to enter a topic to be wiki'd, Senpai!");}
		}
	},
	"hug": {
	...
	"smite": {
		usage: "[user mention]", description: "Smites the mentioned user", delete: true,
		process: function (bot, msg, suffix) {
			if (!suffix) {bot.sendMessage(msg, "**" + msg.sender.name + "** has smited themself using power granted to Bluee by the Cabbage Phoenix.");}
			if(suffix && msg.mentions.length === 1) {bot.sendMessage(msg, msg.mentions[0] + " has been smited using the power granted to Bluee by the Cabbage Phoenix.");}
		}
	},
	"flamethrower": {
		usage: "[no usage]", description: "puts a (╯°□°)╯︵ǝɯɐlℲ", delete: true,
		process: function (bot, msg) {bot.sendMessage(msg, "(╯°□°)╯︵ǝɯɐlℲ");}
	},
	"botserver": {
		usage: "[no usage]", description: "Posts a invite to this bots server", delete: true,
		process: function (bot, msg) {bot.sendMessage(msg, "__**Heres a invite to my server:**__ https://discord.gg/0lBiROCNVaDaE8rR");}
	},
	"sing": {
		usage: "[no usage]", description: "Sings a lovely song", delete: true,
		process: function (bot, msg) {bot.sendMessage(msg, "*🎵sings a beautiful song about Onii-chan🎵*");}
	},
	"weedle": {
		usage: "[no usage]", description: "Weedle Weedle Weedle Wee", delete: true,
		process: function (bot, msg) {bot.sendMessage(msg, "Weedle Weedle Weedle Wee\nhttp://media.giphy.com/media/h3Jm3lzxXMaY/giphy.gif");}
	},
	"letsplay": {
		usage: "[game]", description: "Tells everyone you want to play a game.",delete: true,
		process: function (bot, msg, suffix) {
			if (suffix) {bot.sendMessage(msg, ":video_game: @everyone, **" + msg.author.name + "** would like to play " + suffix + "!");}
			else {bot.sendMessage(msg, ":video_game: @everyone, **" + msg.author.name + "** would like to play a game!");}
		}
	},
	"about": {
		usage: "[none]", description: "Tells you about the bot", delete: true,
	...
	"call": {
		usage: "[none]", description: "Tells everyone you want a call.", delete: true,
		process: function (bot, msg) {bot.sendMessage(msg, ":phone: @everyone, **" + msg.author.name + "** would like to have a call!");}
	},
	"randomquote": {
		usage: "[none]", description: "Gives a random quote from the quotes chat (Doesn't work on most servers)", delete: true, privateServer: true,
		process: function (bot, msg) {
			if (msg.channel.server.id === "87601506039132160") {
	...
	"youtube": {
		usage: "[topic]", description: "Gives you a link to the first result of the searched term.", delete: true,
		process: function (bot, msg, suffix) {
				...
				if (error || !result || !result.items || result.items.length < 1) {bot.sendMessage(msg, "Your search resulted in an error. Please forgive me senpai! ;-;");}
					...
					if (typeof result.items[0].id.videoId === "undefined") {
						for (i = 1; i < result.items.length; i++) {
							if (typeof result.items[i].id.videoId !== "undefined") {bot.sendMessage(msg, "http://www.youtube.com/watch?v=" + result.items[i].id.videoId); return;}
						}
					}
	...
	"avatar": {
		usage: "outputs the avatar of the user of the command or the person linked.", description: "Outputs an avatar url", delete: true,
		process: function (bot, msg, suffix) {
			if (!suffix || !msg.mentions) {bot.sendMessage(msg, "**" + msg.sender.name + "'s** avatar is " + msg.author.avatarURL);}
			if(msg.mentions.length === 1 && suffix) {bot.sendMessage(msg, "**" + msg.mentions[0].username + "'s** avatar is " + msg.mentions[0].avatarURL);}
		}
	},
	"id": {
		usage: "outputs the id of the user of the command or the person linked.", description: "Outputs an id", delete: true,
		process: function (bot, msg, suffix) {
			if (!suffix) {bot.sendMessage(msg, "**" + msg.sender.name + "'s** id is `" + msg.author.id + "`, Senpai")}
			if (msg.mentions.length === 1 && suffix)
			{bot.sendMessage(msg, "**" + msg.mentions[0].username + "'s** User ID is `" + msg.mentions[0].id + "`, Senpai")}
		}
	},
	"anime": {
		usage: "[no usage]", description: "Gives information about the anime mentioned", delete: false,
		process: function (bot, msg, suffix) {
			var anime = msg.content.split(" ").slice(1).join("+");
			var apiURL = "http://myanimelist.net/api/anime/search.xml?q=" + anime;
			if (options.private) {var user = process.env.MAL_user; var pass = process.env.MAL_pass;}
			else {var user = options.MAL_user; var pass = options.MAL_pass;}
	...
	"weather": {
		usage: "[location to get weather from]", description: "Gets the weather", delete: true,
		process: function (bot, msg, suffix) {
			if (suffix) {
				...
				request(rURL, function (error, response, weath) {
					...
					else {console.log(errorC(error)); bot.sendMessage(msg, "There was an error getting the weather, please try again later.");}
				});
			}
			else {bot.sendMessage(msg, "You need to enter a place to get the weather for.");}
		}
	}
}
...
function rssfeed(bot, msg, url, count, full) {
	...
	feedparser.on('readable', function () {
		...
		bot.sendMessage(msg, item.title + " - " + item.link, function () {
			if (full === true) {var text = htmlToText.fromString(item.description, {wordwrap: false,ignoreHref: true});bot.sendMessage(msg, text);}
		});
