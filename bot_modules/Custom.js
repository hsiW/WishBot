    ...
    "chan": {
        ...
        process: function(bot, msg, suffix) {
            if (suffix.split(" ")[0] === "add" || suffix.split(" ")[0] === "write") {
                ...
                if (!CustomCommands.hasOwnProperty(msg.channel.guild.id)) bot.createMessage(msg.channel.id, "No Commands Found");
                    ...
                    var msgString = "Custom Chans - " + Object.keys(CustomCommands[msg.channel.guild.id]).sort().map(cmd => "`" + cmd + "`").join(", ");
...
exports.remove = function(server) {
    ...
    console.log(serverC("@" + server.name + ": ") + botC("@WishBot") + " - " + errorC("Removed from Custom Commands Database"));
    updated = true;
}
