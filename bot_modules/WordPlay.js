...
var prefix = require("./../options/options.json").prefixes[0];

var words = {
"quote": {
  usage: "Prints out a random anime quote or the quote at the position mentioned\n`"+prefix+"quote [number] or [none]`",
  delete: true,
...
"reverse": {
  usage: "Reverses the mentioned terms\n`"+prefix+"reverse [terms]`",
  delete: true,
  process: function (bot, msg, suffix){
    if(suffix.indexOf('@') > 0){suffix = suffix.replace(/@/g,'');}
...
"vquote": {
  ...
  delete: true, privateServer: true,
  process: function (bot, msg, suffix) {
    if (!suffix) {bot.sendMessage(msg, "You'll need to have a quote to quote something, **"+msg.author.username+"**-senpai.");}
